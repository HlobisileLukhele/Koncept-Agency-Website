        ...
        <nav className="container mx-auto px-4 flex justify-between items-center py-4 md:px-16 text-gray-600 text-lg">
          ...
          <div className={`fixed top-0 left-0 w-full h-full bg-white z-50 transition-all duration-300 ease-in-out ${showMenu ? 'opacity-100' : 'opacity-0 pointer-events-none'} md:hidden`}>
            <ul className="nav__list flex flex-col items-left space-y-4 mt-16 text-gray-500 text-6xl text-left">
              <li className="hover:text-gray-500">
                <Link to="./" className="text-2xl text-blue-600" onClick={closeMenu}>
                  Home
                </Link>
              </li>
              <hr />
              <li className="hover:text-gray-700">
                <Link to="/AboutUs" className="text-2xl" onClick={closeMenu}>
                  About
                </Link>
              </li>
              <hr />
              <li className="hover:text-gray-700">
                <Link to="/Services" className="text-2xl" onClick={closeMenu}>
                  Services
                </Link>
              </li>
              <hr />
              <li className="hover:text-gray-700">
                <Link to="/HowWeWork" className="text-2xl" onClick={closeMenu}>
                  HowWeWork
                </Link>
              </li>
              <hr />
              <li className="hover:text-gray-700">
                <Link to="/Projects" className="text-2xl" onClick={closeMenu}>
                  Projects
                </Link>
              </li>
              <hr />
              <li className="hover:text-gray-700 pb-4">
                <Link to="/Contact" className="text-2xl" onClick={closeMenu}>
                  Contact
                </Link>
              </li>
              <hr className="pb-4" />
            </ul>
            <button className="btn-contact px-4 py-2 bg-gray-600 text-white rounded-md focus:outline-none hover:text-gray-200 mt-4 ml-6" onClick={closeMenu}>
              Get In Touch
            </button>
          </div>
        </nav>
